...
- [ ] **net 元件轮廓分离 (sch_wirenet)**: net 掩码混入元件轮廓 (主net 13.8% 粗块,
      336 域)。区分走线 (细长连续) vs 元件轮廓 (闭合粗短) 几何特征, 得纯走线。
      参考: 骨架化 + 线宽众数 (同 net 大部分宽度)。
- [ ] **电容 net 判据 (parameter_space 记录)**: cap_quality 距离惩罚主导, 走线交叉
      处"看起来像电容"。真电容 = 连接两个不同 net 的桥 (板线两侧 net 不同),
      假候选无 net 边界。实现: 板线外侧精确取样 net, 两侧不同 → 强加分。
      sch_wirenet 提供 net 网表 (工具已就绪)。
