...
### 5b-4. 符号识别参数调优规律 (2026-09-17, sch_cap 实测)

**参数调整/优化是独立的复杂问题** —— 与识别算法本身分开对待。完整规律
(约束窗口/多源投票/位置漂移/扫描策略) 见 `tools/sch_symbol/readme.md` §7。
核心结论速记:
- **先参数化, 再调优**: 阈值全走 CLI (architecture §5.9), 否则无法系统扫描
- **评估闭环先行**: 必须有 GT + 量化指标 (命中数/距离分布) 才能判断好坏
- **约束窗口避开被测目标本身**: band 只取板线内侧中央区, 否则真电容被误杀
- **多源投票 > 单源优先**: plates 海量假候选, find_cap_pairs 方向可能误报,
  三源一致性 = 高置信
- **symbol_pos 本身会漂移**: 检测需结合绿线触点/尺寸知识库反查
