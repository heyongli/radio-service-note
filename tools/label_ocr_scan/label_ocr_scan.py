...
def main():
    ...
    ap.add_argument("--limit", type=int, default=0, help="只跑前 N 个 (测试用, 0=全部)")
    args = ap.parse_args()
    ...
    # 过滤候选 (兼容 rectangles / circles 两种裁切索引)
    items_key = 'rectangles' if 'rectangles' in idx else 'circles'
    if args.categories == 'all':
        candidates = idx[items_key]
    else:
        cats = args.categories.split(',')
        candidates = [r for r in idx[items_key] if r['category'] in cats]
    if args.limit > 0:
        candidates = candidates[:args.limit]
    print(f"[label_ocr_scan] candidates: {len(candidates)} (key={items_key}, categories={args.categories})")

    engine = RapidOCR()
    updated_items = list(idx[items_key])
    all_found = []
    ...
    for i, r in enumerate(candidates):
        ...
        if refs:
            ...
            updated_items[rect_idx]['refdes'] = best['refdes']
            updated_items[rect_idx]['status'] = 'ocr_found'
            updated_items[rect_idx]['refdes_source'] = f"label_ocr_scan_rot{best['rot']}_s{best['score']:.3f}"
            print(f"→ {best['refdes']} (rot={best['rot']}, score={best['score']:.3f})")
        else:
            updated_items[rect_idx]['status'] = 'unverified'
    ...
    # 写回 crops_index
    idx[items_key] = updated_items
    idx['_meta']['ocr_scan_version'] = '0.4'
    with open(args.crops_index, 'w') as f:
        json.dump(idx, f, ensure_ascii=False, indent=2)
    print(f"[label_ocr_scan] crops_index.json updated: {args.crops_index}")
    ...
    components['_meta'] = {
        ...
        'consumers': ['tools/ic_ocr_scan/ic_ocr_scan.py', 'tools/svg-render/svg_render.py'],
