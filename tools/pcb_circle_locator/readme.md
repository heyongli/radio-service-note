...
# pcb_circle_locator — PCB 圆形裁切定位器
...
```bash
python tools/pcb_circle_locator/pcb_circle_locator.py \
...
## 与 pcb_rect_locator 配合

```bash
# 1. 矩形检测
python tools/pcb_rect_locator/pcb_rect_locator.py --pcb ... --crops-dir crops/rectangle/

# 2. 圆形检测
python tools/pcb_circle_locator/pcb_circle_locator.py --pcb ... --crops-dir crops/circle/
...
python tools/ic_ocr_scan/ic_ocr_scan.py \
    ...
    --pcb ... --out nettable/pcb_components.json
```
